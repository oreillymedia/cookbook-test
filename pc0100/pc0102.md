...
`record = ('Dave', 'dave@example.com', '773-555-1212', '847-555-1212')`{{execute}}

`name, email, *phone_numbers = record`{{execute}}

`name`{{execute}}

`email`{{execute}}

`phone_numbers`{{execute}}
...
`*trailing, current = [10, 8, 7, 1, 9, 5, 10, 3]`{{execute}}
...
`line = 'nobody:*:-2:-2:Unprivileged User:/var/empty:/usr/bin/false'`{{execute}}

`uname, *fields, homedir, sh = line.split(':')`{{execute}}
